...
export default class RecentsView {
  constructor(state) {
    this.state = state
    this.state.projects = []
    this.updateProjects()
    atom.history.onDidChangeProjects(event => {
      this.updateProjects()
    })
  ...
  render() {
    return (
      <div id='atom-recents' className='landing'>
        ...
        <div className='col projects'>
          <h1 className='projects-header'>Your Recent Projects</h1>
          {this.getProjects().map((project, key) => {
            return (
              <div className='project-card'>
                <div
                  className='project-info icon-repo'
                  onclick={event => this.openProject(key, event)}
                >
                  {project}
                </div>
              </div>
  ...
  openProject(key) {
    projectUtil.switch(this.state.projects[key].paths)
    atom.workspace.closeActivePaneItemOrEmptyPaneOrWindow()
  }

  updateProjects() {
    this.state.projects = atom.history.projects
    this.update()
  }
  ...
  getProjects() {
    ...
    this.state.projects.forEach(function(project) {
      var paths = []
      project.paths.forEach(function(projectPath) {
        paths.push(path.basename(projectPath))
      })
      projects.push(paths.join(', '))
    })
    return projects
  }
